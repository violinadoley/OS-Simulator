...
function signupresponse(event, userinput) {
  var x = event.keyCode;
  if (x == 13) {
    // On ENTER key press (Keycode of ENTER is 13)
    var userin_split = userinput.split(" "); //Tokenize command
    switch (
      userin_split[0] //Checking password for respective users
    ) {
      case "1":
...
function nextline(event, text) {
  ...
  if (x == 13) {
    ...
    for (
      i = 0;
      i < commands.length;
      i++ //Detecting the command (Which command it is)
    ) {
    ...
    if (i < commands.length) {
      //Checking if command is predefined ie., is among the predefined list
      switch (
...
function create(file) {
  for (
    i = 0;
    i < file_dir.length;
    i++ //Traversing through all files to check if filename already exists
  ) {
    if (file_dir[i][0] == file && file_dir[i][2] == pointer) {
      // Second condition verifies whether elements of current directory are checked
      // or not
...
function ls() {
  ...
  for (
    i = 0;
    i < file_dir.length;
    i++ //Getting names of all files present
  ) {
...
function rm(file) {
  for (i = 0; i < file_dir.length; i++) {
    if (
      file_dir[i][0] == file &&
      file_dir[i][2] == pointer &&
      file_dir[i][1] == 0
    ) {
      //3rd Check is to confirm this is a file, not a folder
  ...
  for (i = 0; i < file_dir.length; i++) {
    if (
      ...
      contents.setAttribute(
        "onkeydown",
        "savefile(event,contents.value,index,contents)"
      ); //Add attribute to DOM to send data on each key press
...
function mkdir(dir) {
  for (
    i = 0;
    i < file_dir.length;
    i++ // Check if dirname already exists in current directory
  ) {
    if (file_dir[i][0] == dir && file_dir[i][2] == pointer) {
      // Second condition verifies whether elements of current directory are checked
      // or not
